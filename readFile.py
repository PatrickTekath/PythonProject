...
def readTrainingDocument(pathTraining="train.json"):
    try:
        ...
        print("File could not be found. Loading train.json as default.")
    return trainRecipes

"""
Function to split the training data into a training and a test set
"""
def splitData(splitRatio, extTrainingFile = ''):
    trainRecipes = readTrainingDocument('train.json')
    if(extTrainingFile):
        trainRecipes['ingredients'] = pd.Series.from_csv(extTrainingFile, encoding='utf-8')
...
def processDocuments(split=False, splitRatio=0.25, extTrainingFile='', extTestFile=''):
    ...
    if(split == False):
        ...
        if(extTrainingFile):
            trainingIngredients = pd.read_json('train.json')
            trainingIngredients['ingredients'] = pd.Series.from_csv(extTrainingFile, encoding='utf-8').tolist()
            training, testing = cv.train_test_split(trainRecipes)
